...
/*   Copyright (C) 2015 - 2016 Alexis José Turruella Sánchez
	 ...
	 Fichero tabla_categoria_producto.php
*/
...
// Security check
if(!$user->rights->preciovolumen->aplicartablascategoriaproducto)
{
	accessforbidden();
}
...
if($_POST['accion']=='aplicartablaacategoriaproducto')
{
	$nueva_asociacion = new tabla_categoria_producto($db);
...
if($_GET['accion']=='del')
{
	$del_asociacion = new tabla_categoria_producto($db);
...
$lista_categorias_asociadas = $objPrecioVol->Tarifas_Por_Categoria(0);

$obj_categorias = new Categorie($db);
$lista_categorias_productos = $obj_categorias->get_main_categories(0); 

$html = new Form($db);
$morejs=array("/pvplus/js/preciovolumen.js");
llxHeader('',$langs->trans('ProductsCategoriesShort'),$langs->trans("ProductsCategoriesShort"),'','','',$morejs,'',0,0);

$aTablasPrecios = $objPrecioVol->ListaTablas();
$aProductosTablas = $objPrecioVol->ProductosTablas();

print_fiche_titre($langs->trans("ProductsCategoriesShort"));
...
if(sizeof($lista_categorias_asociadas) > 0)
{
	...
	print '<td>'.$langs->trans('PVTABLAAPLCADA').'</td>';
	...
	print '</label> ';
	print '<a href="tabla_categoria_producto.php?accion=del&id_asociacion='.$asociacion->id.'" style="padding-left:1px" >'.img_picto($langs->trans("PVDELTABLA"),"delete").'</a>';
	print '</td>';
	print '</tr>';
	}
	print '</table>';
...
print '<br/>';
print "<form method='post' action='tabla_categoria_producto.php' name='nuevo'>";
print '<input type="hidden" name="accion" value="aplicartablaacategoriaproducto"/>';
...
print '<tr>';
print '<td>';
print $langs->trans('ProductsCategoriesShort');
print ' <select name="categoria">';
foreach($lista_categorias_productos as $cat)
{
